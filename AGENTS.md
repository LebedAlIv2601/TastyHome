...
- Если feature impl модуль реализует контракт своего feature api модуля, то зависимость на свой api модуль должна подключаться через `api(...)`, а не через `implementation(...)`
