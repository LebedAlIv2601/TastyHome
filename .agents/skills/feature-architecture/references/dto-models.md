...
- Все DTO классы должны быть помечены `@Serializable`, даже если конкретная реализация временно использует моковые данные вместо реального `body<...>()`.
- Отсутствие реальной сетевой интеграции сейчас не является причиной убирать `@Serializable` с DTO: DTO в этом проекте всегда считаются сериализационным data-контрактом.
