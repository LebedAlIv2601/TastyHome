...
- Все request/response DTO, с которыми работает RemoteDataSource, должны быть `@Serializable`, даже если конкретный remote временно замокан и не ходит в настоящую сеть.
